...
**Auth: Supabase OAuth 2.1** (`oauthSupabaseProvider`). Clients authenticate against Supabase; the server verifies the JWT and runs every query with a **request-scoped, RLS-aware** client (the caller's access token), so Postgres RLS enforces tenant isolation — the server holds no service-role data access. Tenant/role come from JWT claims (`tenant_id`, `tenant_role`) injected by `custom_access_token_hook`; `teacher`/`admin` get management tools, `student` gets only the 19 self-scoped learning/practice tools (role gating in `src/tool-policy.ts` + per-tool guards in `src/register.ts`). Audit logging (`mcp_audit_log`) runs via an `mcp:tools/call` middleware using the service-role key (optional; no-op if unset). The OAuth consent UI is hosted at `/auth/consent` (`src/auth-routes.ts`) — set this as the consent screen URL in Supabase Dashboard → Authentication → OAuth Server (standalone/local). In production the Next.js proxy (`app/api/mcp/[[...path]]/route.ts`) fronts the server at `https://<tenant>.<domain>/api/mcp`: clients add that URL as a Claude custom connector, `.well-known/oauth-protected-resource` advertises Supabase as the authorization server, and the consent screen URL points at the Next.js `/oauth/consent` page instead.
...
**Surface:** 84 `lms_*` tools (courses/lessons/exercises/exams/analytics + 8 admin-only landing-page tools (`lms_get_landing_blocks` block vocabulary + page-craft guide, `lms_list/get/create/update_landing_page`, `lms_publish/unpublish/delete_landing_page`) — same json-render → Puck pipeline as the in-app AI builder, vocabulary mirrored from `lib/puck/config.ts` into `mcp-server/src/landing/*.generated.ts` by `npm run gen:puck-fields` (re-run after changing any Puck block; the 3 page tools render the `landing-page-preview` widget — structural wireframe cards per section + link to the real admin preview) + 27 student-scoped tools: 6 learning + 6 AI-tutor practice (`lms_get_exercise_for_student` w/ attempt history, `lms_complete_exercise` host-graded text + real_time_conversation exercises → engine_type 'simulation', `lms_practice_quiz`, `lms_record_practice_attempt`, `lms_get_my_weak_spots`, `lms_get_tutor_config`) + 6 Phase 2 (`lms_get_course_content` paginated bulk ingest, `lms_search_content` snippet search over entitled courses, `lms_get_mock_exam_source` missed-questions+rubrics from own submitted exams ONLY, `lms_get_tutor_history` + `lms_record_tutor_session` shared `aristotle_sessions` memory with the in-app tutor, `lms_enroll_in_course` via `self_enroll_subscription_course` RPC) + 1 Phase 3 (`lms_get_exam_readiness` per-topic mastery + weighted readiness score, 50% exam history / 30% practice / 20% lesson coverage, renormalized); + 7 Phase 4 (flashcards/FSRS: `lms_create_review_cards`, `lms_get_due_reviews`, `lms_grade_review` — FSRS (ts-fsrs) math server-side since #389, `review_cards` RLS own-rows; weekly study plan: `lms_set_study_plan` replace-per-week, `lms_get_study_plan` w/ next-lesson + due-card context, `lms_complete_study_goal` idempotent, `study_goals` table; `lms_ask_teacher` consented escalation → teacher notification via SECURITY DEFINER RPC `create_student_question_notification`, enrollment-validated + rate-limited 3/day/course) + 1 adaptive (#396 `lms_get_adaptive_practice_items` — Elo-ranked exercises targeting ~80% expected success; ratings live in `item_ratings`/`student_topic_ratings`, updated by SECURITY DEFINER DB triggers on practice/exercise/exam-score inserts, `lms_get_my_weak_spots` returns per-topic elo block, hotspots return `hardest_items`); teacher Phase 3 adds `lms_get_confusion_hotspots` (severity-ranked struggle aggregates over practice_attempts + failed exercise_evaluations + exam-question miss rates, `verifyCourseOwnership`-gated) and `lms_duplicate_exercise` (copy as draft w/ overrides for remediation variations)); 18 widgets — `course-dashboard`, `course-detail` (+ live stats), `exam-submissions` (drill-in), `lesson-preview`, `artifact-sandbox` (live iframe preview + edit/save of artifact-exercise HTML, via `lms_preview_artifact`), `submission-grader` (per-question review + score/feedback override, via `lms_get_submission_for_grading` + `lms_grade_submission`), `student-progress-roster` (per-course progress %, exam avg, at-risk flags, via `lms_get_student_progress`), `school-overview` (admin-only cross-course KPI dashboard, via `lms_get_school_stats`), plus 7 student widgets — `my-learning` (via `lms_my_learning`), `lesson-viewer` (via `lms_view_lesson`, mark-complete button calls `lms_complete_lesson`, "I don't understand" button hands off to the tutor), `my-exam-results` (via `lms_my_exam_results`), `gamification-profile` (via `lms_my_gamification`), `course-catalog` (via `lms_browse_catalog`; plan-covered cards get an Enroll button → `lms_enroll_in_course`), `practice-player` (via `lms_practice_quiz`: in-widget answering, closed types graded locally + recorded via `lms_record_practice_attempt`, free-text handed back to the host to grade), `exam-readiness` (via `lms_get_exam_readiness`: readiness dial + topic bars + per-topic "Practice this" buttons), `flashcards` (via `lms_get_due_reviews`: flip-card review, Again/Hard/Good/Easy → `lms_grade_review`, end-screen summary), `study-plan` (via `lms_get_study_plan`: progress ring + kind-grouped checklist → `lms_complete_study_goal`, plan-next-week button); 3 resource templates (`course://`, `lesson://`, `exam://`); 12 prompts (5 authoring incl. generate-remediation-exercises (hotspots → draft exercise variations via `lms_create_exercise`/`lms_duplicate_exercise`, never auto-published) + 7 tutor: socratic-tutor, drill-coach, explain-my-mistake, exam-prep-session, daily-review, conversation-practice, mock-exam; socratic-tutor and exam-prep-session open with `lms_get_tutor_history` and close with `lms_record_tutor_session`). Practice attempts persist to `practice_attempts` (RLS own-rows, +15 XP capped 10/day, `source_exercise_id` links variation drills to the real exercise). Student tools are self-scoped (RLS + explicit user filters), use `has_course_access` RPC for enrollment checks, and respect `require_sequential_completion` locks; `lms_complete_lesson` is the only student mutation (idempotent insert into `lesson_completions`). Tool registration lives in `mcp-server/src/tools/*.ts`; widgets in `mcp-server/resources/<name>/widget.tsx`.
