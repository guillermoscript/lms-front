...
| Doc | What it covers |
|--|--|
| [STATUS.md](./STATUS.md) | **What works today**: the four MVP loops, their specs, and what is blocked on credentials |
...
`docs/archive/` holds dated implementation summaries, phase write-ups, test reports and
schema snapshots — **historical records of a moment in time**, not maintained references.
Several describe schema or files that no longer exist. Use them for context on *why*
something was done; never as the current source of truth. See
[archive/README.md](./archive/README.md) for what is in there and what replaced it.

Also here: `docs/adr/` (architecture decision records), `docs/plans/` and
`docs/refactor-plans/` (planning documents), `docs/refactor-reports/` (what each refactor
did), and `docs/qa/` plus its older siblings `verification/`, `evidence/`, `screenshots/`,
`media/` and `images/` (screenshot evidence attached to pull requests — see
[qa/README.md](./qa/README.md)). `docs/SMOKE_TEST_LOG.md` is regenerated by
`tests/playwright/smoke-test.spec.ts`; do not hand-edit it.

`ENTITLEMENTS_MIGRATION_PLAN.md` stays out of the archive: it is the written record of how
access moved to `entitlements`, and it carries its own status banner.
