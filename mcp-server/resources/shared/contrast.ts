 ...
 *
 * Landing-page blocks let an admin set any colour they like, and widgets paint
 * that colour as a surface (a hero background, a CTA pill) or use it as text on
 * one of our own neutral surfaces (a stat number on a white/zinc-900 card).
 * Both directions need the *other* colour derived from the one we were given —
 * hardcoding `#ffffff` makes a light brand colour illegible, and passing the
 * raw accent through makes a near-black brand colour vanish on a dark card.
 *
 * Everything here is pure sRGB maths on the colour we were handed, with no
 * dependency on the widget theme: `readableOn()` only needs the surface it is
 * writing on, and `accentTextOn()` is called once per candidate surface so the
 * caller can hand both to CSS and let the `dark:` variant choose. That matters
 * because `resources/styles.css` is not part of the widget build (see the
 * warning at the top of that file), so `dark:` resolves to Tailwind's default
 * `prefers-color-scheme` query rather than the `.dark` class the widget sets —
 * picking a surface in JS from `useWidgetTheme()` would drift out of sync with
...
/** The card surface widgets paint in light mode (`bg-white`). */
