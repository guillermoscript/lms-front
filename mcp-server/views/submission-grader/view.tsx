...
function SubmissionGrader() {
  ...
  // useDynamicTool has no isError/error fields (unlike v1's useCallTool) — it
  // throws instead, so the save outcome is tracked locally from the catch.
  const [saveFailed, setSaveFailed] = useState(false);
  const [saveErrorMessage, setSaveErrorMessage] = useState<string | null>(null);

  const { callTool: saveGrade, isPending: saving } = useDynamicTool<
  ...
  const { submission, questions, summary } = view.toolOutput as Props;
  ...
  const handleSave = () => {
    ...
    void (async () => {
      try {
        ...
        setSaveFailed(false);
        setLocalStatus("teacher_reviewed");
      } catch (err) {
        setSaveFailed(true);
        setSaveErrorMessage(err instanceof Error ? err.message : null);
      }
    })();
  };
