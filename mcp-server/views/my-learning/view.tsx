...
function MyLearning() {
  ...
  // The server validated structuredContent against this same schema
  // (`outputSchema` on the tool), so a cast is safe here.
  const { total, average_progress, courses } = view.toolOutput as Props;
                      ...
                      {done ? (
                        ...
                        */
                        <button
                          onClick={() =>
                            void sendFollowUp({
                              prompt: `Open lesson ${course.next_lesson!.id} ("${course.next_lesson!.title}") from "${course.title}" with lms_view_lesson.`,
                            })
                          }
