...
function CourseDetail() {
  ...
  const loadStats = useDynamicTool<{ course_id: number }, Record<string, unknown>>(
    "lms_get_course_stats"
  );
  const statsLoading = loadStats.isPending;
  // useDynamicTool's handle has no isError flag — a populated `error` is the
  // same signal.
  const statsFailed = loadStats.error !== undefined;
  const statsData = loadStats.data;
  ...
  const { course, lessons, exams } = view.toolOutput as Props;
  ...
  const handleLoadStats = () => {
    ...
    void loadStats.callTool({ course_id: course.id });
  };
