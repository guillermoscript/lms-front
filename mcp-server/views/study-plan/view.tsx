...
import { type Props, type Goal } from "./schema";
...
function StudyPlan() {
  ...
  const { week_start, goals, context } = view.toolOutput as Props;
        ...
        <div className="flex flex-col gap-1.5">
          {context.next_lessons.slice(0, 3).map((l) => (
            <button
              ...
              onClick={() =>
                void sendFollowUp({
                  prompt: `Open lesson ${l.lesson_id} ("${l.lesson_title}") from "${l.course_title}" with lms_view_lesson.`,
                })
              }
