...
import { type Props, type Question } from "./schema";
...
function PracticePlayer() {
  ...
  const { callTool: recordAttempt, isPending: isRecording } = useDynamicTool<
  ...
  // re-read from props on every render until questions actually arrive.
  const props = view.status === "ready" ? (view.toolOutput as Props) : undefined;
  ...
  const { topic, questions } = props as Props;
  ...
  const isMixed = (props as Props).mode === "mixed";
  ...
  const finish = () => {
    ...
    if (hasFreeText) {
      ...
      if (!handedOff) {
        ...
        const freeSummary = results
          ...
          .join("\n");
        void sendFollowUp({
          prompt: `I finished the ${isMixed ? "mixed (interleaved) " : ""}practice quiz on "${topic}".\n\nWidget-graded questions (${closed.filter((x) => isCorrect(x, effectiveAnswer(x))).length}/${closed.length} correct):\n${closedSummary || "(none)"}\n\nFree-text answers for you to grade:\n${freeSummary}\n\nGrade my free-text answers, compute the overall score, record it with lms_record_practice_attempt${isMixed ? " (mode 'mixed', keep each question's topic tag so per-topic attribution survives)" : ""}${(props as Props).source_exercise_id ? ` (source_exercise_id ${(props as Props).source_exercise_id})` : ""}${(props as Props).lesson_id ? ` (lesson_id ${(props as Props).lesson_id})` : (props as Props).course_id ? ` (course_id ${(props as Props).course_id})` : ""}, then reteach anything I got wrong.`,
        });
      }
      return;
    }
    ...
    const handOff = (saved: boolean) => {
      ...
      void sendFollowUp({
        prompt:
          (missed.length === 0
            ? `I scored ${correctCount}/${closed.length} on the "${topic}" practice quiz — all correct! Suggest what to practice next${(props as Props).source_exercise_id ? `, or whether I'm ready to complete the real exercise (id ${(props as Props).source_exercise_id})` : ""}.`
            : `I scored ${correctCount}/${closed.length} on the "${topic}" practice quiz. I missed:\n${missed.map((m) => `- ${m.prompt} (I answered: ${m.answer})`).join("\n")}\n\nReteach me what I got wrong, then quiz me again${(props as Props).source_exercise_id ? ` (keep source_exercise_id ${(props as Props).source_exercise_id})` : ""}.`) +
          unsavedNote,
      });
    };
    ...
    void (async () => {
      try {
        await recordAttempt({
          ...
          ...((props as Props).course_id !== null
            ? { course_id: (props as Props).course_id }
            : {}),
          ...((props as Props).lesson_id !== null
            ? { lesson_id: (props as Props).lesson_id }
            : {}),
          ...((props as Props).source_exercise_id !== null
            ? { source_exercise_id: (props as Props).source_exercise_id }
            : {}),
