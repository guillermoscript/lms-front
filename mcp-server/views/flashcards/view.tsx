...
function Flashcards() {
  ...
  const { callTool: gradeReview } = useDynamicTool<
    { card_id: number; rating: Rating },
    unknown
  >("lms_grade_review");
  ...
  const { cards, total_due } = view.toolOutput as Props;
  ...
  const rate = (rating: Rating) => {
    ...
    void gradeReview({ card_id: card.id, rating }).catch(() =>
  ...
  if (finished) {
    ...
    // every other sendFollowUp payload in these widgets.
    const summary =
        ...
        <div className={dark ? "dark" : ""}>
          <div className={container}>
            <div className="rounded-xl border border-zinc-200 bg-white p-8 text-center dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <button
                onClick={() => void sendFollowUp({ prompt: summary })}
