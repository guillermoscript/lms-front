...
function AttemptCounter({ used, max }: { used: number; max: number }) {
...
function RetryPanel({ score, passingScore, onRecordAgain }: {
  score: number
  passingScore: number
  onRecordAgain: () => void
}) {
  ...
  return (
    <div className="rounded-2xl border-2 border-primary/15 bg-gradient-to-br from-primary/[0.04] to-primary/[0.01] p-6 space-y-5">
      ...
      <Button onClick={onRecordAgain} className="w-full gap-2.5 h-12 text-sm font-bold tracking-wide">
        <IconVideo size={18} />
        {t('recordAgain')}
...
function CollapsibleFeedbackSummary({ evaluation }: { evaluation: SpeechEvaluation }) {
  const t = useTranslations('exercises.video')
...
function CompletionSummary({ score, passingScore, completedDate }: {
  score: number
  passingScore: number
...
export default function VideoExercise({
  ...
  const showRetryPanel = evaluation && !showRecorder && passed !== true && !dailyLimitReached
  const showFeedbackSummary = evaluation && showRecorder && passed !== true
...
function VideoSubmissionHistoryRow({ submission, attemptNumber, passingScore }: {
