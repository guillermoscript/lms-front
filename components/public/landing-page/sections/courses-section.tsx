...
import type { CoursesSectionData, SectionColors } from '@/lib/landing-pages/types'
...
export async function CoursesSection({ data, accentColor = '#3B82F6', colors }: Props) {
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto">
          {products.map((product) => (
            <article key={product.product_id} className={`${cardBg} border ${cardBorder} rounded-2xl overflow-hidden hover:border-zinc-700 transition-colors`}>
              {product.image && (
              ...
              <div className="p-5 space-y-3">
                <h3 className={`font-bold ${headingColor}`}>{product.name}</h3>
                {product.description && (
                  <p className={`${bodyColor} text-sm leading-relaxed line-clamp-2`}>{product.description}</p>
                )}
