import type { TestimonialsSectionData, SectionColors } from '@/lib/landing-pages/types'
...
export function TestimonialsSection({ data, colors }: Props) {
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-5xl mx-auto">
          {(data.items ?? []).map((item, idx) => (
            <blockquote key={idx} className={`${cardBg} border ${cardBorder} rounded-2xl p-6 space-y-4`}>
              <p className={`${bodyColor} leading-relaxed italic`}>"{item.quote}"</p>
              <footer className="flex items-center gap-3">
                ...
                <div>
                  <p className={`${headingColor} font-semibold text-sm`}>{item.name}</p>
