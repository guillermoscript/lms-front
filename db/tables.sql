...
create table public.exercise_files (
  ...
  content text not null,
  file_type public.exercise_file_type not null default 'code'::exercise_file_type,
  constraint exercise_files_pkey primary key (id),
  constraint unique_file_per_exercise unique (exercise_id, file_path),
  constraint exercise_files_exercise_id_fkey foreign KEY (exercise_id) references exercises (id) on update CASCADE on delete CASCADE
) TABLESPACE pg_default;

create table public.exercise_code_student_submissions (
  id bigserial not null,
  exercise_id bigint not null,
  user_id uuid not null,
  submission_code text not null,
  created_at timestamp with time zone null default CURRENT_TIMESTAMP,
  constraint exercise_code_student_submissions_pkey primary key (id),
  constraint exercise_code_student_submissions_exercise_id_fkey foreign KEY (exercise_id) references exercises (id) on update CASCADE on delete CASCADE,
  constraint exercise_code_student_submissions_user_id_fkey foreign KEY (user_id) references auth.users (id) on delete CASCADE
) TABLESPACE pg_default;