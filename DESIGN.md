...
- **Shape:** `rounded-button` (`--radius-button`, 8px by default); the `xs` and `icon-xs` sizes take 75 percent of it (6px). A theme kit sets it to 2px (sharp) or a full pill (round), so a `Button` never carries a stylistic `rounded-*` of its own; only an intentional circle (avatar trigger, floating action) may override it. Buttons, cards and inputs have their own tokens; menus, tabs, popovers, dialogs and tooltips stay on `--radius: 0.625rem` so a round theme never turns them into pills.
