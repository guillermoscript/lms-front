...
export function paymentInstructionsTemplate(
  ...
  return {
    subject: `Payment instructions for ${data.itemName} — ${data.schoolName}`,
    html: `<!DOCTYPE html>
<html>
<body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1a1a1a">
  <h2 style="color:#2563eb">How to pay for ${item}</h2>
  <p><strong>${school}</strong> has sent the payment instructions for your request.</p>
  <table style="border-collapse:collapse;margin:16px 0">
    <tr><td style="padding:4px 12px 4px 0;color:#666">Amount</td><td style="padding:4px 0"><strong>${escapeHtml(data.amountLabel)}</strong></td></tr>
    ${data.paymentMethod ? `<tr><td style="padding:4px 12px 4px 0;color:#666">Method</td><td style="padding:4px 0">${escapeHtml(data.paymentMethod)}</td></tr>` : ''}
    ${data.deadlineLabel ? `<tr><td style="padding:4px 12px 4px 0;color:#666">Pay by</td><td style="padding:4px 0">${escapeHtml(data.deadlineLabel)}</td></tr>` : ''}
  </table>
  <div style="background:#f5f5f5;border-radius:6px;padding:16px;margin:16px 0;line-height:1.5">${instructions}</div>
  <p>Once you have paid, upload your receipt on your request page so the school can confirm it.</p>
