...
import { GRADING_SECRETS_EMBED, mergeGradingSecrets } from '@/lib/exercises/grading-secrets'
import {
...
export async function loadLessonCheckpoints(
  ...
  const { data: checkpoints } = await adminClient
    .from('lesson_checkpoints')
    .select(
      `id, placement_type, content_block_id, video_timestamp_seconds, label, allow_skip, max_ai_attempts, is_required, exercises(id, title, description, instructions, exercise_type, exercise_config, tenant_id, ${GRADING_SECRETS_EMBED})`
    )
  ...
  for (const row of checkpoints) {
    ...
    const config = mergeGradingSecrets(exercise.exercise_config, exercise.exercise_grading_secrets)
    result.push({
      id: row.id,
