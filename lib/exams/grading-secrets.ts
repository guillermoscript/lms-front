...
/** `Q` with every option under `options` / `question_options` carrying `is_correct`. */
type WithOptionFlags<Q> = {
  [K in keyof Q]: K extends 'options' | 'question_options'
    ? Q[K] extends Array<infer O> | null | undefined
      ? Array<O & { is_correct: boolean }> | Extract<Q[K], null | undefined>
      : Q[K]
    : Q[K]
}

function flagOptions(options: OptionLike[], correctIds: number[]) {
  return options.map((o) => ({ ...o, is_correct: correctIds.includes(o.option_id) }))
}

function withOptionFlags<Q extends QuestionLike>(question: Q, correctIds: number[]): WithOptionFlags<Q> {
  const merged: QuestionLike = { ...question }
  ...
  return merged as WithOptionFlags<Q>
}
...
export function withExamGradingSecrets<Q extends QuestionLike & { exam_grading_secrets?: unknown }>(
  question: Q
): WithOptionFlags<Omit<Q, 'exam_grading_secrets'>> & Omit<ExamGradingSecretsRow, 'correct_option_ids'> {
  const { exam_grading_secrets: embed, ...rest } = question
  const row = secretsRow(embed)
  return {
    ...withOptionFlags(rest as Omit<Q, 'exam_grading_secrets'>, row?.correct_option_ids ?? []),
...
): Array<WithOptionFlags<Q> & { correct_answer: string | null }> {
